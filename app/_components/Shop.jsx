...
import { useDecor } from "../_hooks/Decor"
import { useRef } from "react"
import {useGLTF, useScroll} from "@react-three/drei"
import { useFrame } from "@react-three/fiber"
import { useGrid } from "../_hooks/useGrid"
...
const ShopItem=({map,item,...props})=>{
    ...
    return <group {...props}><group position={gridToVector3([0,0],size[0],size[1])}><primitive object={clone}/></group></group>
}
export const Shop=({onItemSelected})=>{
   ...
   const shopItems= useMemo(()=>{
        ...
        return Object.values(items).map((item,index)=>{
            ...
            maxX.current=x;
            return (
                <ShopItem map={map} key={index} position-x={xPos} item={item} onClick={(e)=>{e.stopPropagation(); onItemSelected(item)}}/>
            )
    })
    },[items])
    ...
    return <group ref={shopContainer}>
        {shopItems}
    </group>
}