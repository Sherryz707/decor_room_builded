...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "SET_ITEMS":
      ...
      case 'REMOVE_ITEMS':
        console.log("in reducer case",action.payload,state.map.items.filter(
          item =>
...
export const DecorProvider = ({ children }) => {
    ...
    state.map.items.forEach((item)=>{
      ...
      for(let x=0;x<width;x++){
        for(let y=0;y<height;y++){
          state.grid.setWalkableAt(item.gridPosition[0]+x,item.gridPosition[1]+y,false);
  ...
  const generateRandomPosition=()=>{
    ...
    for (let i = 0; i < 100; i++) {
      const x=Math.floor(Math.random()*state.map.size[0]*state.map.gridDivision)
      const y=Math.floor(Math.random()*state.map.size[1]*state.map.gridDivision)
  ...
  return (
    <DecorContext.Provider value={{ state, dispatch,deleteItems,updateItems,generateRandomPosition,setPosition,findPath,setInvItems }}>
      {children}
